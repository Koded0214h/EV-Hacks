...
import { api, mapZone, mapStation, mapROIResult, getToken, setToken, clearAuth, setCachedUser, getCachedUser } from '../api.js'
...
function AIBrief({ zone, lastROI }) {
  ...
  const exportPDF = async () => {
    ...
    doc.text('EV HACKS — INVESTOR BRIEF', MARGIN, 9)
    doc.text(new Date().toLocaleDateString('en-NG', { year: 'numeric', month: 'long', day: 'numeric' }), W - MARGIN, 9, { align: 'right' })
    y = 28
    ...
    doc.setFont('helvetica', 'bold')
    const headlineLines = doc.splitTextToSize(brief?.headline ?? `${zone.name} — Investment Brief`, TW)
    doc.text(headlineLines, MARGIN, y)
    y += headlineLines.length * 8 + 3

    // Zone tag line
    doc.setFontSize(9)
    doc.setTextColor(22, 163, 74)
    doc.setFont('helvetica', 'bold')
    doc.text(`${zone.name}  ·  Tier ${zone.tier}  ·  Score ${zone.score}/100`, MARGIN, y)
    y += 10
    ...
    const summaryLines = doc.splitTextToSize(brief?.summary ?? `${zone.name} scores ${zone.score}/100 on EV demand (Tier ${zone.tier}).`, TW)
    doc.text(summaryLines, MARGIN, y)
    y += summaryLines.length * 5.5 + 10

    // Key Metrics
    const metrics = brief?.key_metrics ?? [
      { label: 'Demand Score', value: `${zone.score}/100` },
      { label: 'Competition',  value: `${zone.existing} stations` },
    ...
    metrics.forEach((m, i) => {
      ...
      doc.text(m.label ?? m.lbl, MARGIN + colW * i + colW / 2, y + 5.5, { align: 'center' })
    })
    y += 8
    metrics.forEach((m, i) => {
      doc.setFontSize(11)
      doc.setFont('helvetica', 'bold')
      doc.setTextColor(15, 23, 42)
      doc.text(String(m.value ?? m.val ?? ''), MARGIN + colW * i + colW / 2, y + 6.5, { align: 'center' })
    })
    y += 14
    ...
    if (brief?.risk_factors?.length) {
      ...
      y += 6
      brief.risk_factors.forEach(r => {
        doc.setFontSize(9)
        doc.setFont('helvetica', 'normal')
        doc.setTextColor(220, 38, 38)
        doc.text('▸', MARGIN, y)
        doc.setTextColor(51, 65, 85)
        const lines = doc.splitTextToSize(r, TW - 6)
    ...
    if (brief?.recommendation) {
      const recLines = doc.splitTextToSize(brief.recommendation, TW - 12)
      const recH = recLines.length * 5 + 12
      doc.setFillColor(240, 253, 244)
      doc.rect(MARGIN, y, TW, recH, 'F')
      doc.setFillColor(22, 163, 74)
      doc.rect(MARGIN, y, 3, recH, 'F')
      doc.setFontSize(8)
      doc.setFont('helvetica', 'bold')
      doc.setTextColor(22, 163, 74)
      doc.text('RECOMMENDATION', MARGIN + 7, y + 5)
      doc.setFontSize(9)
      doc.setFont('helvetica', 'normal')
      doc.setTextColor(15, 23, 42)
      doc.text(recLines, MARGIN + 7, y + 10)
      y += recH + 8
    }
    ...
    doc.setFont('helvetica', 'normal')
    doc.text('Generated by EV Hacks · Nigeria EV Infrastructure Intelligence', MARGIN, 292)
    doc.text('evhacks.ng', W - MARGIN, 292, { align: 'right' })

    doc.save(`EV-Hacks-Brief-${zone.name.replace(/\s+/g, '-')}-${Date.now()}.pdf`)
  }
      ...
      {state === 'done' && (
        <div className="dp-brief-result">
          ...
          <div className="dp-brief-actions">
            <button className="dp-brief-export" onClick={exportPDF}><Download size={13}/> Export PDF</button>
...
function InvestorSignup({ onBack, onDone }) {
  ...
  const submit = async () => {
    ...
    setErr(''); setLoading(true)
    try {
      const data = await api.auth.register(form.email.trim(), form.password, form.name.trim(), { company: form.company, role: form.role })
      setToken(data.access)
      setCachedUser(data.user)
      onDone(data.user)
    } catch (e) {
